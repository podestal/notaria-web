...
const CartasNotariales = () => {
  ...
  const { data: cartasNotariales, isLoading, isError, error, isSuccess, refetch } = useGetIngresoCartas({ access, page:1, dateFrom, dateTo })
  ...
  return (
    <div className="mt-[80px] w-[85%] mx-auto bg-slate-100 rounded-lg shadow-lg mb-10 text-black py-6">
        ...
        <ExtraProtocolaresHeader 
          ...
          refetch={refetch}
        />
        <>{console.log('cartasNotariales', cartasNotariales)}</>
    </div>
  )
}
