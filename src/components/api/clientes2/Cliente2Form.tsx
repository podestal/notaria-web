        ...
        const handleSubmit = (e: React.FormEvent) => {
            ...
            updateCliente2.mutate({
                ...
                cliente: {
                    ...
                    telofi: officePhone,
                    sexo: gender === 1 ? 'M' : 'F',
                    idestcivil: civilStatus || 0,
                    natper: '',
                    conyuge: '', 
                    nacionalidad: nationality?.id || '',
                    idprofesion: profesion?.id ? parseInt(profesion.id) : null,
                    detaprofesion: profesion?.label || '',
                    idcargoprofe: cargo?.id ? parseInt(cargo.id) : 0,
                    profocupa: cargo?.label || '',
                    dirfer: direccion,
                    idubigeo: ubigeo?.id || '',
                    cumpclie: birthdate,
                    razonsocial: '',
                    fechaing: new Date().toISOString().split('T')[0],
                    residente: resident === 1 ? 'Sí' : 'No',
                    tipocli: 'N',
                    profesion_plantilla: '',
                    ubigeo_plantilla: '',
                    fechaconstitu: '',
                    idsedereg: 0, // Assuming no branch data
