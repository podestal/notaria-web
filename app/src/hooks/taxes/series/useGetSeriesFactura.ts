import { useQuery, type UseQueryResult } from "@tanstack/react-query"
import {
    facturaSeriesService,
    type SerieControlInterno,
} from "../../../services/taxes/seriesService"
import { normalizeTaxesList } from "../../../services/taxes/normalizeTaxesList"

interface Props {
    access: string
    enabled?: boolean
}

const useGetSeriesFactura = ({
    access,
    enabled = true,
}: Props): UseQueryResult<SerieControlInterno[], Error> => {
    return useQuery({
        queryKey: ["taxes-series-factura"],
        queryFn: async () =>
            normalizeTaxesList(await facturaSeriesService.get(access)),
        enabled: enabled && !!access,
    })
}

export default useGetSeriesFactura
