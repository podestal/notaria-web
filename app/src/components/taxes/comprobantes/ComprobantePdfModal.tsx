...
import { fetchIngresoPdfBlob } from "../../../services/taxes/ingresosService"
import { fetchReciboPdfBlob } from "../../../services/taxes/recibosService"
...
const ComprobantePdfModal = ({ variant, item, onClose }: Props) => {
    ...
    useEffect(() => {
        ...
        const loadPdf = async () => {
            ...
            try {
                const id = getComprobanteItemId(item)
                const blob =
                    variant === "ingreso" && isIngreso(item)
                        ? await fetchIngresoPdfBlob(id, access)
                        : await fetchReciboPdfBlob(id, access)
    ...
    const itemId = getComprobanteItemId(item)

    const handlePrint = () => {
        const iframe = document.getElementById(
            `comprobante-pdf-${itemId}`,
        ) as HTMLIFrameElement | null
        iframe?.contentWindow?.print()
    }

    return (
        <TopModal isOpen={Boolean(item)} onClose={onClose} portal wide deepth={70}>
