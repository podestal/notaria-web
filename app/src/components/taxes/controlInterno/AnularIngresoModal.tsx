...
import { getIngresoBackendError } from "./ingresoFormShared"
...
const AnularIngresoModal = ({ ingreso, onClose, onSuccess }: Props) => {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setMessage("Ingreso anulado correctamente")
            setType("success")
            setShow(true)
            onSuccess?.()
            onClose()
            ...
            <div className="mx-auto w-full max-w-lg rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 flex gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
                    ...
                    <p className="text-xs leading-relaxed text-amber-900">
                        Una vez anulado, el ingreso quedará registrado como baja y no
                ...
                <form onSubmit={handleSubmit} className="mt-5 space-y-4">
                    ...
                    <div className="flex flex-wrap justify-end gap-2 border-t border-slate-100 pt-4">
                        <button
                            type="button"
                            onClick={onClose}
                            disabled={anularIngreso.isPending}
                        ...
                        >
                            {anularIngreso.isPending ? "Anulando…" : "Confirmar anulación"}
                        </button>
                    </div>
                </form>
            </div>
...
export default AnularIngresoModal
